...
export function scanPullRequest(input: ScanInput, options: { failOn?: RiskLevel } = {}): ScanReport {
...
function summarize(score: number, findingCount: number): string {
  if (findingCount === 0) return "No policy findings detected.";
  if (score >= 85) return "Critical review risk detected. Do not merge until resolved.";
  if (score >= 60) return "High review risk detected. Maintainer approval should be required.";
  if (score >= 30) return "Moderate review risk detected. Ask for evidence before merge.";
